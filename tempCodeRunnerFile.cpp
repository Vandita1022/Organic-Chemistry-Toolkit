    ...
    void parseMolecularFormula(const string& formula) {
        ...
        for (size_t i = 0; i < formula.size();) {
            ...
            if (ch == 'C') {
                ...
                // Parse following hydrogens like H3
                ...
                // ✅ Handle halogens as attached to the last carbon
                if (halogenLabel == "Cl" || halogenLabel == "Br" || halogenLabel == "F" || halogenLabel == "I") {
                    if (previousCarbon != 0) {
                        carbons[previousCarbon].incrementC_X();
                    }
                    i++;
                } else {
                    // Unrecognized element — treat it like carbon
